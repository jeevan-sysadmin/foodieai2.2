...
export default function Chat() {
  ...
  useEffect(() => {
    setChatMessages([
      ...
      { text: 'Search for Your Favorite Food Tips', isUser: true }
    ])
  }, [])
  ...
  const handleSearch = async () => {
    ...
    try {
        ...
        console.log('Sending request to API with query:', inputCode);

        // Construct the URL for the API request
        const url = new URL('http://13.234.51.189:8000/recipe/' + encodeURIComponent(inputCode));
        console.log(url)
        ...
        // Extract recipe steps or handle response accordingly
        const message = data.steps ? 
            `Steps for ${inputCode}: \n${data.steps.join('\n')}` :
            'No steps found for the given recipe. Please try another query.';
  ...
  return (
    <Flex w='100%' h='500px' direction='column' position='relative' px={{ base: 4, md: 0 }} justifyContent='center'>
      <Box
        flex='1'
        p={{ base: 4, md: 8 }}
        borderRadius='25px'
...
const ChatBubble = ({ text, isUser }: ChatMessage) => {
  return (
    ...
    >
      ...
      {!isUser && <CustomAvatar alt='user-profile' src='/images/img/Foodie.png' variant='rounded' size={40} />}
